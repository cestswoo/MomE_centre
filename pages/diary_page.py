...
def main():
    ...
    # 사이드바 메뉴
    menu = ["Home", "Dashboard", "Diary", "Mom:ents", "하루 자가진단", "LogOut"]
    icons = ['bi bi-house-fill','bi bi-grid-1x2-fill','book-half','bi bi-star-fill' ,'bi bi-capsule-pill', 'box-arrow-in-right']
    
    # 역할에 따라 표시할 메뉴 항목을 선택
    if st.session_state['role'] == "산모":
        allowed_menu = menu
    elif st.session_state['role'] == "남편":
        allowed_menu = ["Home", "Diary", "Mom:ents", "LogOut"]
    
    with st.sidebar:
        selected_menu = option_menu("MomE", allowed_menu,
                                    icons=[icons[menu.index(m)] for m in allowed_menu],
    ...
    if selected_menu == 'Home':
        ...
        st.write("대시보드 페이지입니다.")
    elif selected_menu == 'Diary':
        st.write("일기장 페이지입니다.")
        show_diary_page()
    elif selected_menu == 'Mom:ents':
        st.write("Mom:ents 페이지입니다.")
    elif selected_menu == '하루 자가진단':
        st.write("자가진단 페이지입니다.")
